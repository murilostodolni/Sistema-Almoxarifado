...
public class SaidaProdutoController {
	...
	private ArrayList<Produto> listaSaida = new ArrayList<>();
	...
	public void adicionarProdutoBanco() {
		...
		for (int i = 0; i < listaSaida.size(); i++) {
			try {
				ProdutoDAO.getInstance().saidaProduto(listaSaida.get(i), listaSaida.get(i).getTotalProdutos(),
						IniciarCena.usuarioAtual);
				saida += listaSaida.get(i).getTotalProdutos() + " x " + listaSaida.get(i).getNomeProduto() + "\n";
			} catch (ProdutoSemEstoqueException e) {
				try {
					Produto emEstoque = ProdutoDAO.getInstance().buscar(listaSaida.get(i).getCodigoBarra());
					Alert alerta = new Alert(AlertType.ERROR);
					alerta.setTitle("Erro!");
					alerta.setHeaderText("O produto " + listaSaida.get(i).getNomeProduto() + "(Código: "
							+ listaSaida.get(i).getCodigoBarra() + ")" + " possui " + emEstoque.getTotalProdutos()
							+ " em estoque!");
					alerta.setContentText("NÃO FOI DADO SAÍDA DE NENHUM produtos");
					alerta.show();
		...
		if (!saida.equals("")) {
			...
			alerta.setTitle("Saída bem sucedida!");
			alerta.setHeaderText(saida);
			alerta.show();
		}
		atualizarLista();
	}
	...
	public void atualizarLista() {
		...
		tblListaProdutos.setItems(FXCollections.observableArrayList(listaSaida));
	}
}